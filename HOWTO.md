...
sample-16       180    1           1
```
The degrees of affine rotation is the degrees of counter-clockwise rotation that it takes to bring a coronal slice into the same orientation as the template to be registered to. Additionally, to flip the hemisphere of a slice, set the value in the `flip` column to 1, as with sample-16 above. These will all be inputs to the `processVisiumData` or `processMerfishData` functions. ***Text of image will be updated soon, the images showing -180 rotation are now 180 + flip***

...
geneListFromTed = ['Hnrnpdl', 'Hnrnpa2b1', 'Rbm3', 'Homer1']
...
for i, regSample in enumerate(processedSamples):
    ...
    plt.axis('off')
    plt.savefig(os.path.join(derivatives,f'geneExpression{actGene}{processedSamples[i]["sampleID"]}.png'), bbox_inches='tight', dpi=300)
...
#%% generate mean and t-stat for specific set of genes
...
for geneIndex,actGene in enumerate(ht5Genes):
    ...
    try:
        ...
        tStatFig = axs[2].scatter(templateDigitalSpots[:,0],templateDigitalSpots[:,1], c=np.array(actTstats), cmap='seismic',alpha=0.5,vmin=-4,vmax=4,plotnonfinite=False,marker='.')
...
for listIdx,actGene in enumerate(ht5Genes):
    ...
    try:
        ...
        # display mean gene count for control group
        axs[0].imshow(template['rightHemAnnotEdges'],cmap='gray',aspect="equal")
        nsdFig = axs[0].pcolormesh(yEdges, xEdges, avgControlDensity.T, cmap='Reds', vmax=heatmapMax, alpha=0.7)
        axs[0].set_title('NSD')
        axs[0].axis('off')
        nsdCb = fig.colorbar(nsdFig,fraction=0.046, pad=0.04)
        nsdCb.ax.tick_params(labelsize=5)
        axs[1].imshow(template['rightHemAnnotEdges'], cmap='gray')
        sdFig = axs[1].pcolormesh(yEdges, xEdges, avgExperimentalDensity.T, cmap='Reds', vmax=heatmapMax, alpha=0.7)
        axs[1].set_title('SD')
        axs[1].axis('off')
        sdCb = fig.colorbar(sdFig,fraction=0.046, pad=0.04)
        sdCb.ax.tick_params(labelsize=5)
        axs[2].imshow(template['rightHemAnnotEdges'], cmap='gray')
        tTestFig = axs[2].pcolormesh(yEdges, xEdges, tTestHeatmap.T, cmap='seismic', alpha=0.7)
        axs[2].set_title('t-statistic of density\n SOR > HC')
        axs[2].axis('off')
        tTestCb = fig.colorbar(tTestFig,fraction=0.046, pad=0.04)
        tTestCb.ax.tick_params(labelsize=5)
        plt.suptitle(f'Mean density of spots expressing {actGene} per ${cellSize}\mu m^2$\n p={minP}')
        plt.savefig(os.path.join(derivatives,f'cellDensityDifference{actGene}.png'), bbox_inches='tight', dpi=300)
        plt.show()
        plt.close()
...
for listIndex,actGene in enumerate(geneListFromTed):
    digitalSamplesControl = np.zeros([nDigitalSpots,(nSampleControl * kSpots)])
    digitalSamplesExperimental = np.zeros([nDigitalSpots,(nSampleExperimental * kSpots)])
    startControl = 0
    stopControl = kSpots
...
sigGenesWithPvals = []
sigGenesWithTstats = []
for geneIndex,actGene in enumerate(geneListFromTed):
    digitalSamplesControl = np.zeros([nDigitalSpots,(nSampleControl * kSpots)])
    digitalSamplesExperimental = np.zeros([nDigitalSpots,(nSampleExperimental * kSpots)])
    startControl = 0
    stopControl = kSpots
...
for geneIndex,actGene in enumerate(geneListFromTed):
    ...
    try:
        ...
        print(f'{actGene} not in list')
        continue
#%% extract data for Isocortex
plt.close('all')
regionOfInterest = 'Isocortex'
roi = stanly.createRegionalMask(template, regionOfInterest, hemisphere='rightHem',displayImage=True)
roiSpots = stanly.createRegionalDigitalSpots(roi, template, spotSize, hemisphere='rightHem', displayImage=True)
...
nDigitalSpots = len(roiSpots)
#%%
# spot-by-spot regional t-test of Isocortex using Sidak correction
start_time = time.time()
desiredPval = 0.05
alphaSidak = 1 - np.power((1 - desiredPval),(1/(len(allSampleGeneList))))
rankList = np.arange(1,nDigitalSpots+1)
...
#%% generate mean and t-stat for specific genes in Isocortex
plt.close('all')
start_time = time.time()
desiredPval = 0.05
alphaSidak = 1 - np.power((1 - desiredPval),(1/(len(allSampleGeneList))))

...
# create list of spots in best sample rather than template
...
for nOfGenesChecked,actGene in enumerate(allSampleGeneList):
    ...
    # digitalSamplesControl[digitalSamplesControl == 0] = np.nan
    # digitalSamplesControl[digitalSamplesControl == 0] = np.nan
    digitalSamplesExperimental = np.array(digitalSamplesExperimental, dtype=float).squeeze()
    # digitalSamplesExperimental[digitalSamplesExperimental == 0] = np.nan
    # digitalSamplesExperimental[digitalSamplesExperimental == 0] = np.nan
    ...
    if sum(checkAllSamples) < 20:
        ...
        # alphaSidak = 1 - np.power((1 - 0.05),(1/sum(checkAllSamples)))
        ...
        allPvals = []
    # if/else here will skip gene if there are fewer than X spots of shared expression
    # will this cause a problem for regions with changed but not insignificant expression? 
    # if len(testControlSamples) < 20:
    #     continue
    # else:
        ...
        if sum(mulCompResults) > spotThr:
            ...
            # fdrMask = actTtest[0] < mulCompResults[3]
            # maskedDigitalCoordinates[fdrMask == True] = inTissueTemplateSpots[fdrMask == True, :]
            # maskedFdrValues = []
            # maskedFdrValues[fdrMask == True] = actTtest[1][fdrMask == True]
            # for idx in enumerate(mulCompResults[0]):
            #     if idx[1] == True:
            #         maskedDigitalCoordinates.append(inTissueTemplateSpots[idx[0],:])
            #         maskedFdrValues.append(actTtest[0][idx[0]])
            ...
            # plt.imshow(bestSampleToTemplate['visiumTransformed'],cmap='gray')
            # plt.scatter(inTissueTemplateSpots[:,0],inTissueTemplateSpots[:,1], c=np.array(meanDigitalControl), alpha=0.8, vmin=-2,vmax=2,plotnonfinite=False,cmap='seismic')
            # # plt.title(f'Mean gene count for {actGene}, control')
            # plt.title(f'Z-score of gene count for {actGene}, non sleep deprived')
            # plt.colorbar()
            # plt.savefig(os.path.join(derivatives,f'meanGeneCount{actGene}Control.png'), bbox_inches='tight')
            # plt.show()
            
            # plt.imshow(bestSampleToTemplate['visiumTransformed'],cmap='gray')
            # plt.scatter(inTissueTemplateSpots[:,0],inTissueTemplateSpots[:,1], c=np.array(meanDigitalExperimental), alpha=0.8, vmin=-2,vmax=2,plotnonfinite=False,cmap='seismic')
            # plt.title(f'Z-score of gene count for {actGene}, sleep deprived')
            # plt.colorbar()

...
close all; clear; clc
addpath('readObj');
heData=fullfile('/','home','zjpeters','rdss_tnj','otherLabs','visium','Visium_HE_sections'); % location of tifs
templateData = fullfile('/','home','zjpeters','rdss_tnj','otherLabs','visium','templates');
templateImage = imread(fullfile(templateData,'tinyimage.jpg'));
...
d3=(6.5/9000) * 0.1;  %mm/pixel resolution of template image, 9000 pixels between border, scaled to match later data
...
upperLeftLoc = templateImage(720:1319,530:1229,:); ul = imresize(upperLeftLoc, 0.1);
upperRightLoc = templateImage(720:1319,10350:11049,:); ur = imresize(upperRightLoc, 0.1);
lowerLeftLoc = templateImage(10750:11349, 530:1229,:); ll = imresize(lowerLeftLoc, 0.1);
lowerRightLoc = templateImage(10750:11349, 10350:11049,:); lr = imresize(lowerRightLoc, 0.1);
templateImage = imresize(templateImage, 0.1);
%% detect features
...
binF = bwareafilt(bin, [0,1000]);   % filter out the brain section
binF = double(binF);
%% prepare fiducials
...
for i = 1:length(sortImPad)
    Vbw(:,:,i)=sortImPad{i};     %size corrected images concatenated into a 3d volume
end
%%
% se=strel('disk',5);

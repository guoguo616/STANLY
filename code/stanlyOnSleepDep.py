...
hippoMask = stanly.createRegionalMask(template, 'Hippocampal formation')
hippoDigitalSpots = stanly.createRegionalDigitalSpots(hippoMask, 12)

allSampleGeneList = allSamplesToAllen[0]['geneListMasked']
for i, regSample in enumerate(allSamplesToAllen):        
    actNN, actCDist = stanly.findDigitalNearestNeighbors(hippoDigitalSpots, allSamplesToAllen[i]['maskedTissuePositionList'], kSpots, spotSize)
...
nDigitalSpots = len(hippoDigitalSpots)
...
for nOfGenesChecked,actGene in enumerate(geneList):
    ...
    if sum(checkAllSamples) < 20:
        ...
        testSpotCoordinates = hippoDigitalSpots[checkAllSamples,:]
        ...
        if sum(mulCompResults) > spotThr:
            ...
            maskedDigitalCoordinates = hippoDigitalSpots[np.array(mulCompResults)]
            ...
            plt.imshow(bestSampleToTemplate['visiumTransformed'],cmap='gray')
            plt.scatter(hippoDigitalSpots[:,0],hippoDigitalSpots[:,1], c=np.array(meanDigitalControl), alpha=0.8, vmin=0,vmax=maxGeneCount,plotnonfinite=False,cmap='Reds',marker='.')
            ...
            plt.imshow(bestSampleToTemplate['visiumTransformed'],cmap='gray')
            plt.scatter(hippoDigitalSpots[:,0],hippoDigitalSpots[:,1], c=np.array(meanDigitalExperimental), alpha=0.8, vmin=0,vmax=maxGeneCount,plotnonfinite=False,cmap='Reds',marker='.')
            ...
            plt.imshow(bestSampleToTemplate['visiumTransformed'],cmap='gray')
            plt.scatter(hippoDigitalSpots[:,0],hippoDigitalSpots[:,1], c=np.array(actTtest[0]), cmap='seismic',alpha=0.8,norm=zeroCenteredCmap,plotnonfinite=False,marker='.')
            plt.title(f't-statistic for {actGene}.')
            plt.colorbar()
            plt.savefig(os.path.join(derivatives,f'tStatGeneCount{actGene}SleepDepHippocampal.png'), bbox_inches='tight', dpi=300)
            plt.show()


timestr = time.strftime("%Y%m%d-%H%M%S")
with open(os.path.join(derivatives,f'listOfSigSleepDepGenesHippocampus{timestr}.csv'), 'w', encoding='UTF8') as f:
    writer = csv.writer(f)
    for i in sigGenes:
        writer.writerow([i])
...
templateDigitalSpots = stanly.createDigitalSpots(bestSampleToTemplate, spotSize)
for i, regSample in enumerate(allSamplesToAllen):        
